...
Screenshots of what was built, in `docs/reports/0.13-reading-layer/`, each with a text cartoon beside it so the report survives a moved image. The design mockups in `docs/long-prompts/images/` are drawings of what was *intended* and are not overwritten.

### The split view

![The reading pane on a filed paper: the page left with its notes marked, the discussion right](0.13-reading-layer/fig-split-view.png)
...
![Documents, Nodes with its filter open, Contents, Library, Sessions and the development shelf](0.13-reading-layer/fig-side-panel.png)
...
![What is being written to against what is being shown, with the rename and remove verbs](0.13-reading-layer/fig-session-selector.png)

![A session's permalink: every annotation filed in it, oldest first](0.13-reading-layer/fig-session-permalink.png)
...
![A box floating over the page at its mark](0.13-reading-layer/fig-placement-floating.png)

![The margin column beside the page](0.13-reading-layer/fig-placement-margin.png)

![A box inline in the flow of a node page](0.13-reading-layer/fig-placement-inline.png)
...
![A proposal as the page beside its rendering](0.13-reading-layer/fig-proposal-box.png)
...
![A filed paper with no way into the reader: links out to the raw PDF and nothing else](0.13-reading-layer/i1-fault-no-way-in.png)

*F2, before the fix: the work's own page offered no way to open it in loom.*

![The composer reporting that nobody is attached while the agent is working](0.13-reading-layer/i1-busy-dispatch.png)

*F7: "nobody is attached — it waits in the inbox. Start one with: loom session watch …", said while the agent it names is mid-task.*

![A box over the page showing the resolve it just fired](0.13-reading-layer/i1-box-updates-in-place.png)

*F3, after the fix: the box says `resolved` and offers `undo resolve`, without being closed and reopened.*

![A 1984 scan anchored by text](0.13-reading-layer/i3-scan-annotated.png)
