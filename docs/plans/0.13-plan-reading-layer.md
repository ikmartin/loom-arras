...
- **The orphan document the showcase should heal is not built.** §12 asks for one, and the design's *an orphaned document stays reachable, and heals* (`refs scan` offering an entry for any stored document no entry names, using the copy ledger) was never implemented — only its tombstone, `refs forget`, was. The showcase cannot show it, and does not pretend to.
...
- **There is no `transcript.jsonl`.** The plan's §3 layout names one beside `inbox.jsonl`; what exists is the inbox plus the session's journal and command log, which is what the surfaces actually read. Either the layout drops it or something should be writing it.
