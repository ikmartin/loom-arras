...
- **Four `arras` e2e tests fail on `main` and still fail** — they expect `/refs/arxiv/…/paper.pdf` while the fixture manifest says `digests/storage/…`. The DR-192 rename left the expectations behind.
