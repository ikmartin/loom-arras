...
**[decided]** The PDF invariant (item 1), as amended by DR-198: `digest extract` refuses a source outside the store; `refs build`'s extract step skips a work with **no source** and reports it blocked (a work with LaTeX source and no PDF extracts, and the lint says what it lacks is a page); a lint code, never an error, reports renderable content with no readable copy; `refs build` closes the gap by fetching where consent exists. `loom refs unreadable CITEKEY --why` and `loom refs forget <citekey|hash> --why` write `digests/unreadable.json`, refuse under an agent identity, and suppress the lint while appearing in the build report's own section.
...
- **[resolved]** A stacked tick is an **expansion in place**: one box holding every lead comment (`fragments/expand.ts::leadComments`, `openAt`), on a page as in a fragment; a menu would have been a second click to reach what one click can show.
- **[resolved]** The sidecar carries **bodies for neither**: they stay in the manifest for the Authoring View and for works alike (DR-209, the author's decision of 2026-09-21), and only geometry goes beside it. WQ-36 keeps the move for a manifest that is actually large.
- **[resolved]** `--wait` defaults to **120 seconds** (`cli/session.py`), under the tool timeouts of the harnesses tried; the orientation says to keep it under whatever the harness allows.
- **[resolved]** `loom session watch` **keeps its place**: it is the presence heartbeat, which is what lets the composer say honestly whether anybody is listening, and a tail that consumes no cursor; the viewer's stream is neither.
...
**Figures** — the split view, the side panel, the session selector, the delete confirmation, the three placements, and the proposal box's new arrangement, as screenshots of **what was built**, in the report's own directory `docs/reports/images/0.13-reading-layer/`, with their text cartoons beside them so the report survives a moved image. The design mockups in `docs/long-prompts/images/` are a drawing of what was intended and are never overwritten: where the built thing differs from its mockup, the report shows both and says why.
...
**2026-09-21 · A page of a cited work is a target (DR-209, DR-210).** The hole the audit found — every piece around §1's sentence built and the sentence itself not — is closed. `Anchor` is one type in `loom/anchors.py`, shared by results and annotations; an annotation's `anchor` carries the page fields beside the text triple, told apart by `kind`, which `Selector.from_dict` used to swallow silently. A text anchor records offsets and derives its rectangles at build time; a box records the rectangles. Both resolve against the store: `recorded` is the artifact's hash, `detached` a quotation that no longer locates, a box never. `loom comment CITEKEY --page N --quote|--box`, over the API and in a batch, through one function — `refs/anchoring.py::anchor_on_page` — that the `locate` preview also calls, so the endpoint and the record cannot disagree; a test asserts that with one comparison. `status` gives these notes no row and no count, `--reading` lists them by work, `ai findings` carries the citekey and the page. The sidecar carries their rectangles under `marks` beside the results' `quads`, the page table a real rotation from `pdfinfo`, and the reference a `reading` count. **Bodies stay in the manifest**, by the author's decision, deviating from the design's per-work sidecar.

In the viewer: `NoteAt.svelte` opens at the selection with loom's own words as the quote before anything is typed; notes draw coloured by kind, on drawn pages only; two on one place are one mark carrying the count, and the box that opens holds both — which the first trial found by accident, two notes on one phrase intercepting each other's click; the same controller opens the box over the page that opens it in a fragment, given a host; a margin column stands beside the page. `Reading` is content only and the Library View is one split with Paper and Digest tabs — the frame inside a frame is gone. A `cited:` link with a copy here lands there, in the app's own locator keys (`page`, `span`, `box`, `annot`), the place lit while the URL carries it; the hover card on such a link shows the page at the place; the paper's own links are followed. Sessions are named and closed from the page. The panel has its Nodes section and its collapse; the tick column stands beside annotated lines in fragments and beside pages. Zoom is per renderer kind; `inline` is the Authoring View's alone; the slot helpers are one module and the margin column reaches every route. The fake agent parked on `session next` is the repository's first backgrounded-process test, and a reading suite under `loom serve` on a showcase copy covers what the fixture-only suite cannot. The box tool could not draw over an existing mark until that suite said so.

**2026-09-21 · What the tests found on the way.** Four defects, none of which a reading of the code had shown. **The box tool could not draw over an existing mark**: the press landed on the mark, which took it, and no box began — marks, links and ticks are pointer-transparent while the tool is active. **A drag began and never ended**: the pointer was captured on the page div while the handlers were on the text layer, so every later event went where nothing listened; capture is on the layer that listens. **An arrow key from the middle did nothing**: a step of 0.02 landed inside the snap's 0.03 and was pulled straight back; the snap is for a pointer, whose aim is approximate, and a key names an exact step. **Two notes on one phrase intercepted each other's click**, which is the stacked case §8 decides a count for, and is now one mark. And one finding about the tests themselves: at Playwright's default viewport a page of a paper at the default zoom runs past the scroll column, and a drag on a clipped point lands on whatever is drawn there — the reading suite runs at the harness's 1440×900 and scrolls the column to the place first. The Nodes section, the tick column, the paper's own links and the page in the hover card are in; the plan's §15 questions are marked resolved with what resolved them; book 7, 8, 10, 11, 12, 15, both specs, the agent documents and the appendices describe what is; DR-209 and DR-210 record the two decisions.

**2026-09-21 · The reading study, three iterations (§17), and the report (§18).** Papers of three characters: a pdfTeX arXiv paper, an OCR'd 1988 journal scan, and a 1984 scan with no source at all. The executor played the author through `scripts/drive.mjs`; a Sonnet agent attached to the same session, parked on `session next`, was dispatched to from the composer, answered, proposed a result, and asked for a verification through a `suggestion` when `refs verify` refused it. **Thirteen faults, twelve fixed**, each with a test. Three were major and none was visible from the code: a filed paper with no digest had **no way into the reader** at all; an annotation box **never showed the write it had just made**, so the reader clicked resolve twice and the log took two events; and `is_agent` failed on `Referee (Agent)` — the form loom's own orientation and showcase use — so a parked agent showed as `⟨person⟩` and that name **ran `loom refs unreadable`, exit 0**, against a verb DR-185 reserves for the author. Iteration 2 found that a document stating no identifier is filed under its content hash while the manifest looks under the entry's synthetic one, so the reader says "No copy of this paper on this machine" about a paper in the store — every scan and most pre-arXiv literature. Iteration 3 produced only a column collision, which is §17's stopping condition, so the loop stopped at three.

What the study confirmed is worth as much as what it broke: twenty drags across the worst text layer in the corpus mapped **19 to text, 1 to a box, none refused**, and the one box spans a running header and a folio, where geometry is the honest record; the page table follows a scan's varying page sizes; the cold resume lost no round marker, cursor or anchor across a stop, a rebuild and a date boundary; and a Sonnet agent found reading, annotating, proposing and the suggestion route from `loom ai orient` alone. The report is `docs/reports/0.13-reading-layer.md`, with eight figures of what was built and four of what was wrong, and it ends with seven sittings for the author — each naming the decision it would overturn — because a loop with no person in it settles nothing about whether any of this is pleasant.
...
- **The showcase now shows the orphan, the page notes and the agent's reply.** `gen_quilts.py` drops a hand-written one-page PDF into `refs/`, scans, deletes its entry and scans again, so the adopted entry is in the checked-in bibliography; the reading session carries a text note and a box note on Bellamy p.2, the author's message with the changed block, the agent's reply, its note on the page and its `suggestion` asking for a verification; the `cited:` links are in the query form. `loom status` on it shows none of the reading notes and `--reading` lists three. What the showcase still cannot show is the three placements, which are a viewer preference and not data.
