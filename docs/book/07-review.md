...
**[decided]** **Accepting an external node claims its transcription is faithful, never that this corpus proved it** (DR-172). External nodes owe no proof and are proved by acceptance alone (7.6.3), and accepting one gates nothing downstream — a key that depends on it is settled whether or not anyone accepted it. What it buys is a seal: the author checked loom's copy against the paper on a date, and the acceptance goes stale the moment that copy moves. So `loom accept` reports `verified KEY as a faithful transcription of CITEKEY`, `status` and the review panel read `transcription verified`, and the stale cause is `transcription-changed` rather than `own-text-changed`. The state in the ledger and the manifest is still `accepted`; only the words differ, and a viewer that shows the manifest's own label for every other key needs no change.
