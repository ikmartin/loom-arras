...
1. Watch: poll mtimes of every `.tex`, `.sty`, `.cls`, and `.bib` file under the root outside `build/` — the canon directory included, since a landmark is rendered though it is never scanned — of `config.toml`, of the acceptance ledger and the history under `.loom/`, of the annotation log under `annotations/`, and of the run files under `ai/` once a second (as sitegen does); on change, run `loom build`, whose render cache limits re-rendering to the fragments whose inputs changed (the changed file's nodes, their masters, and any fragment whose marks depend on a changed record), then publish (M2: a new manifest 0.88 s after an edit).
