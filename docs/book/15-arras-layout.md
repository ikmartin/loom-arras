...
**[decided]** **The split** is a mode of a route rather than a route of its own (DR-202): content on one side, discussion on the other, one divider between them, on the Authoring View, a node page, a work's Library View and a session permalink — and absent from the graph and the review table, which navigate into it instead. **One global ratio**, kept in the viewer's preferences, because a reader sets the shape of their screen once and not once per route. A **single snap at the middle**, since a snap at a third would be a guess about a screen this code cannot see; double-click resets, the arrow keys nudge, and chevrons on the divider collapse either pane while the ratio is remembered. The side panel collapses independently and goes first. **Below about 700px the split becomes a switch**, because two panes on a phone are two unusable panes. Swapping sides is a settings toggle and is expected to be deprecated once one side is known to be right.
...
**[decided]** The **side panel** holds Documents, Nodes and the **Library**, which lists each cited work as a whole document rather than as loose nodes, with the open document's contents under its row and the write target pinned beneath (15.2.3). What loom is **writing to** and what the page is **showing** are said in two places, because they are two questions: the panel's footer names the write target, and **the annotation filter** — `showing annotations from [this session | all]` — stands at the left of the rail above every reading view, because it governs which annotations a document draws rather than which sessions are listed. Writing lands in the selected session whatever is being shown, and a viewer that moved the write target when the view changed would file work where it was not meant to go. **The filter governs the page** — a mark is drawn for a visible annotation and the count a shared place carries is of visible ones, because a filtered page can otherwise look lightly annotated when it is not. **The rail above a reading view** belongs to the shell: the layout draws it once, 36px, over a document, a node, a cited work and a session, and a page contributes its own controls to it (`ReadingActs`) rather than drawing a control row of its own. Tables, the graph and home have no rail.
...
**[decided]** A settings control, in arras's own settings and written to arras's preferences, offers: the shell (15.2), body typeface (serif or sans), body size (three steps), line width (three steps), theme (light, dark, system), the setting a document is read in (`p1`, `p2`, `b1`, `b2` — 15.3.8), and where comments stand (margin, inline or hover, 15.3.1). **[decided]** The four settings are named by their letters rather than described, so the row fits beside the others; each button's title says what it is. Nothing else is user-adjustable in the MVP. Each is one row of the panel, its label and its options on a single line — `Shell  [rail] [strip]` — with the labels in a column of fixed width so that every row's options begin at the same place.
...
ReadingRail()   drawn by the layout above a reading route; filled through ReadingActs
```
