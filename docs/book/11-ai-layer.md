...
3. Vendor files: `CLAUDE.md` and `AGENTS.md` at the quilt root, which carry one line of loom's and are otherwise the author's (DR-151): "This directory is a quilt managed by loom. Before doing anything, run `loom ai orient` and read `ai/rules.md`; follow both. Write only under `ai/runs/`." **[decided]** It names both documents because `loom ai orient` prints the orientation and not the rules, and a mode reached through a slash command passes through neither. Claude Code reads the first, Codex the second. **[decided]** Both stay identical one-sentence copies: Claude Code supports `@path` imports, but `AGENTS.md` readers may not, so neither file imports `ai/orientation.md` by reference (M6).
