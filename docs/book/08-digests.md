...
**[decided]** Mechanical production of a digest from the reference paper's LaTeX source (its e-print unpacked under `refs/<work-id>/src/` by `loom refs fetch`, or any local copy). It is `loom import` applied to someone else's paper with proofs dropped and ids prefixed by the citekey's slug:
...
**[decided]** Since plan 0.12 the cited works have a view of their own, `/digest` (10, DR-184): every cited work with how many of the author's keys cite it, how many results it has and how many proposals wait on the author, filtered in the URL. A work's page carries its asserted links at the top, then its proposals, each as 8.14's verification surface, then the digest. The graph view no longer draws a node that is in a digest and in no document.
...
**[decided]** **Reading.** `loom refs coverage [FRAGMENT]` says what is known of each cited work — source, PDF, pages, sections, digest, proposals waiting — and a fragment of an author's name or a title finds the citekey. `loom refs find TEXT` searches digested statements, grouped by work; `loom refs grep TEXT` searches the page text of every mapped work for a literal phrase, capped per work; `loom refs page CITEKEY N[-M]` prints pages, and is the sanctioned read; `loom refs overview CITEKEY` prints a digest's overview. Every one takes `--run` and logs to that run's `run.log`, and every list says when it truncated.
...
**[decided]** **Verifying** is the author's, in the digest view or with `loom refs verify ID [--statement …] [--local …]`, and it claims that the copy is faithful, nothing mathematical (DR-177). The surface shows both texts — the page around the quotation, with the quotation marked, beside the rendering — and **the page itself**, rendered from the local PDF at build time for every pending proposal and opened at the statement, because the text layer keeps one `X` for a stack and its space and drops every index (DR-179). The author may correct the rendering or the paper's name for the result before verifying; `statement` changes and `source_text` never does, so the anchor stays re-checkable, and both parties are recorded. Re-verifying brings the digest's node back to its record. `loom refs discard ID --reason …` keeps the reason, which `propose` returns to whatever proposes the same result again. `loom refs why ID` prints the provenance and the author's edit as a diff; `loom ai findings --run RUN` prints what the author decided about a run's proposals. `loom refs recheck` re-reads every verified anchor and reports what moved; it never re-verifies anything, and it never re-checks a verified node's LaTeX.

**[decided]** **Linking.** `loom refs link --from ID --to ID --kind same-notion|generalises|specialises|depends-on|contradicts --why … --run RUN` records a relation between two results, with its reason; a link is asserted, never checked and never citable, and is attributed to the run that wrote it. `loom refs links [ID] [--depth N]` walks them. Links are what let a later question be answered from earlier work: in the study an agent traced a chain across four papers in 12 calls, against 46 from the papers.
