...
1. Producing a digest when extraction cannot: only a PDF exists, or the source declares its results in a way the extractor does not see (8.5). The agent reads `refs/pdf/<citekey>.pdf` (or its extracted text) or the fetched source under `refs/src/<citekey>/` and writes `ingest-<citekey>.tex` in the run directory following this chapter's format, with `method: ingest`; `loom ai promote` copies it to `refs/<citekey>.tex` (`--replace` overwrites an existing digest after showing the diff) and lint runs on the result. The mode's instructions require: full statements including hypotheses, verbatim where the source is available; the paper's own numbering as locators; a `-setup` node; `\uses` where the paper states dependencies; macro-free LaTeX with a macro block for the residue; prefixed labels; no proofs (M7: Romagny, seven results and a setup node, promoted).
