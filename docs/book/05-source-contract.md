...
1. **[decided]** Every file with extension `.tex` under the quilt root is scanned, at any depth, except files under `build/` and files with `% !LOOM ignore` in their first twenty lines.
2. **[decided]** Files reached from a master through `\input`, `\nest`, or `\include` are scanned in the context of that master (5.9). Files reached from no master are scanned on their own and reported as loose.
3. **[decided]** Local `.sty` and `.cls` files at the root that a master's preamble loads are scanned for declarations only (5.5) and for macro definitions (Chapter 9). They never contain nodes.
4. **[decided]** The scanner is a restricted parser, not a TeX interpreter. It recognizes the constructs in this chapter by their surface form and refuses or reports what it cannot recognize. Constructs it does not recognize are prose.

## 5.2 Nodes

A node is one of:

- a theorem-like environment: `\begin{ENV}` ... `\end{ENV}` where `ENV` is declared by `\newtheorem` (or `\declaretheorem`) in the master's preamble closure (5.5);
...
3. **[decided]** `\begin{ENV}` and `\end{ENV}` must each be alone on their line (leading whitespace permitted; a trailing comment permitted). The optional argument may follow `\begin{ENV}` on the same line and may span lines if it is bracket-balanced. `import` and `atomize` refuse files that violate this, naming the lines.
4. **[decided]** Environments may nest: a lemma inside a proof, a remark inside a theorem. The scanner walks a tree. A theorem-like environment must be entirely inside one file; one that opens in one file and closes in another is an error `loom:environment-spans-files`.
...
A label is id-shaped if it matches `id`. Since `paperlocal` is broad, the scanner decides which form applies by prefix: a prefix that is a citekey in the quilt's bibliography takes `paperlocal`; any other prefix takes `loomlocal`. A label like `lem:res-indep` is not id-shaped (it contains a colon). A label like `sec-intro` is id-shaped by grammar (`sec` prefix, `intro` local) only if `sec` is a citekey; otherwise `intro` fails `loomlocal` and the label is an alias. **[assumed]** This rule; it keeps human labels with hyphens from being mistaken for ids.
...
- an untagged sectioning unit: `<file>#<label>` if it has any label, else `<file>#section:<n>`;
...
1. **[decided]** Taxa are discovered from the preamble closure of each master: the master's preamble, every `\input` before `\begin{document}`, and every `\usepackage` that resolves to a local file at the root. The scanner does not read system packages.
2. **[decided]** Recognized declarations: `\newtheorem{ENV}{Name}`, `\newtheorem{ENV}[counter]{Name}`, `\newtheorem{ENV}{Name}[section]`, `\newtheorem*{ENV}{Name}`, and thmtools' `\declaretheorem[name=Name, ...]{ENV}` and `\declaretheorem{ENV}` (name defaults to `ENV` capitalized). The taxon is `Name`. The environment name `ENV` is the author's private abbreviation; two authors writing `\begin{lem}` and `\begin{lemma}` produce nodes of the same taxon `Lemma`.
3. **[decided]** The style class is the argument of the most recent `\theoremstyle{...}` before the declaration, defaulting to `plain`. thmtools' `style=` key is honoured.
4. **[decided]** Style class `plain` means the node owes a proof; `definition` and `remark` mean it does not.
5. **[decided]** When a master's preamble closure declares no environment named `ENV` but a node uses it, lint reports `loom:unknown-environment` (error) and the fallback is a directive in the master: `% !LOOM environment: ENV = Name, plain` (one per line). This is the only taxon configuration.
...
- A `plain` node with no attached proof, no `\incomplete`, and no citation in its title is a gap: `loom:missing-proof` (warning).
- A `plain` node with no proof whose title contains `\cite` is an external node (Chapter 8), not a gap.
...
**[decided]** A `proof` environment is attached to a node by exactly one of:

1. Adjacency: it immediately follows a theorem-like node in the same file (only blank lines, comments, or directives between `\end{ENV}` and `\begin{proof}`), and its optional argument contains no `\ref`.
2. Reference: its optional argument contains `\ref{X}` (or `\cref`, `\autoref`) where `X` is an id or alias of a theorem-like node; it may be anywhere in the quilt. Example: `\begin{proof}[Proof of Theorem~\ref{rl-0003}]`.

A `proof` that fits neither is `loom:unattached-proof` (error). A `proof` whose optional argument references several nodes attaches to the first and lint reports `loom:multi-target-proof` (warning).
...
**[decided]** An edge goes from a region (a statement, a proof, or master prose) to a node. Its sources:

1. `\ref{X}`, `\eqref{X}`, `\cref{X}`, `\Cref{X}`, `\autoref{X}`, `\pageref{X}`, where `X` resolves to a node's id or alias, or to a labelled region inside a node (5.8), in which case the edge goes to that node.
2. `\uses{X, Y, ...}`: each item resolves as above. `\uses` is the way to record a dependency the text does not express.
3. `\cite[postnote]{citekey}` where a digest for `citekey` exists and the postnote matches the locator of one of its nodes (Chapter 8). The edge goes to that digest node.

### 5.7.2 Classification

1. **[decided]** An edge is a statement-edge if it occurs in a statement's own text, a proof-edge if in a proof's own text, and a prose-edge if in master prose or an untagged section's own text.
...
2. **[decided]** A reference to a node that the referencing master does not reach is `loom:reference-to-loose` (error for masters, since the PDF will have an undefined reference; info for loose files).
...
Edges from this proof: to `rl-0002` (uses), to `rl-0004` (through the alias), to `Man12-thm-4.1` (postnote, if the digest exists; otherwise `loom:unmatched-postnote`, warning).
...
1. `\input{path}`: the file at `path` (with `.tex` appended if absent), root-relative, is spliced at that point.
...
2. **[decided]** Sectioning units are computed on the expanded text of each master, because sectioning is linear: a `\subsection` in the master after an `\input` line belongs to the section the input file opened. The scanner expands a master with a source map (file and offset per character) and computes sectioning levels with level shifts applied.
3. **[decided]** Files reached from no master are sectioned per file.
4. **[decided]** A file reached from two masters may have different sectioning contexts in each; regions are computed per master.
5. **[deferred]** The representation of the source map and its cost on a large master. Implement a list of (file, start offset, length) spans; measure on the ACGS fixture.

### 5.9.3 Ownership

**[decided]** A node owns its region minus its children's regions (its own text). The master owns the preamble and every character outside all sections. Every character of scanned text belongs to exactly one own text. Annotations anchor into own text; hashing (5.13) uses own text with inclusions replaced by their inclusion lines.
...
4. **[decided]** An `\input` whose file does not exist is `missing-include` (error).
...
Digests are specified in Chapter 8. For the source contract, a digest node is an external node: a `plain` environment with no proof whose title contains `\cite`, with an id of the form `<citekey>-<paperlocal>`, living in a file with a `% !LOOM digest:` header. Its labels and `\eqref`s are prefixed with the citekey at extraction time so that digests from different papers coexist in one bundle.
...
1. Inclusion lines (`\input`, `\nest`, `\include`) are kept as written; the included text is not part of the parent's own text.
2. Comment lines that are not directives are removed; directives are kept.
...
Lint is the set of checks the scanner performs on every scan; diagnostics are published in the manifest (Chapter 9) and printed by `loom lint`. The full code table is `specs/diagnostics.md`. Reserved codes (any publisher): `duplicate-id`, `dangling-link`, `missing-include`, `double-inclusion`, `inclusion-cycle`, `unreachable`. Loom's own codes, with severity:

- error: `loom:environment-spans-files`, `loom:unattached-proof`, `loom:unknown-environment`, `loom:reference-to-loose`, `loom:macros-unloaded`, `loom:duplicate-label`, `loom:line-anchoring` (a `\begin` or `\end` not alone on its line, reported by `import`/`atomize`)
- warning: `loom:missing-proof`, `loom:multi-target-proof`, `loom:equation-in-proof-referenced`, `loom:unmatched-postnote`, `loom:taxon-conflict`, `loom:prefix-is-citekey`, `loom:macro-shadowed`, `loom:unknown-directive`, `loom:version-mismatch`, `loom:missing-package`
- info: `loom:unlabelled-node`, `loom:positional-proof-key`, `loom:unexpected-proof`, `loom:uses-missing`, `loom:uses-unused`, `loom:documentclass-outside-drafts`, `loom:retired-ledger-key`, `loom:undigested-citekey`, `loom:detached-annotation`
...
Keys: `rl-0040`, `rl-0041`, `rl-0042`. The graph shows that `rl-0004` is used only by the first proof; if the author adopts the second, `rl-0004` may become unnecessary, which `loom unravel rl-0004` reports.
...
- Whether nested theorem-like environments (a lemma inside a proof) should be reported as information, since some journals object. **[assumed]** No diagnostic.
