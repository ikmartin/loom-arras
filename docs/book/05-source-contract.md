...
4. **[decided]** The style class controls presentation and never determines proof obligation. On every live scan, loom classifies each theorem-like block's basis as `expository`, `local-proof`, `cited-result`, `assumption`, `open-claim`, or `unclassified` (DR-198, DR-199). Environment names and display taxa `definition`, `defn`, `def`, `notation`, `construction`, `convention`, `remark`, `comment` suggest `expository`; `theorem`, `thm`, `lemma`, `lem`, `proposition`, `prop`, `corollary`, `cor`, `claim`, `fact` suggest `local-proof`; `assumption`, `axiom`, `postulate`, `hypothesis` suggest `assumption`; `conjecture`, `conj`, `question`, `problem` suggest `open-claim`. An explanatory remark needs no separate proof. A remark or comment making a mathematical claim with its justification inline may explicitly declare `local-proof`; this makes the block its own proof, so accepting it covers claim and argument together, and its dependencies are read from the whole block. A result attributed by a citation with a nonempty locator in its title or as the first body token is `cited-result`, whatever its style; every digest result is `cited-result`. Unknown names, conflicting name signals, an attributed result without a locator, or a non-local-proof block with an attached proof become `unclassified` unless the author declares a basis explicitly. `% !LOOM basis: CATEGORY` inside the block overrides inference; a declared `cited-result` still requires a locator, and a digest cannot declare a different basis. `loom:needs-classification` names live unresolved blocks, and the review panel shows them. The author's declaration persists in drafting source, not canon.
...
- A `local-proof` block with no attached proof and no `\incomplete` is a gap: `loom:missing-proof` (warning), regardless of its TeX style. An explicitly classified remark or comment with its justification inline is its own proof and has no missing-proof diagnostic (DR-199).
...
- A legacy `% !LOOM basis: definition` directive is read as `expository` and reported with that migration in `basis_reason`; newly written directives use `expository` (DR-199).

For a block whose environment name is inconclusive, the author records the choice inside the drafting block, where the next scan reads it:

```tex
\begin{remark}\label{rl-0012}
...
2. Adjacency: it immediately follows a theorem-like node among its siblings in the same file (only blank lines, comments, or directives between `\end{ENV}` and `\begin{proof}`), or immediately follows a proof that itself attached by adjacency, in which case it attaches to the same statement (5.6.2). An intervening `remark` or `comment` environment is skipped: the proof attaches to the preceding statement if the gaps contain only whitespace or comments. A proof directly after a remark with no preceding statement is unattached; a proof explicitly naming the remark still attaches by rule 1 (DR-199).
