...
The check made for this revision, on 2026-09-16, with the collected test ids against the two code tables: the 47 codes of `docs/specs/diagnostics.md` and the 47 codes of `loom/src/loom/scan/diagnostics.py` are the same set; 46 of them are emitted somewhere in loom's source, and the forty-seventh, `loom:interface-version`, is emitted by arras (`src/lib/manifest/loader.ts`) as the specification says and is tested by its loader spec; `test_all_emitted_codes_are_known` keeps every code the fixture quilts emit inside the table. Two codes gained their first test at M7, `loom:documentclass-outside-drafts` and `loom:bundle-failed`, both in `test_documentclass_outside_drafts_and_bundle_failed`. Three loom codes still have no test that asserts them, `loom:atomize-target-exists`, `loom:foreign-annotations`, and `loom:main-not-found`, and `test_non_utf8_source_decoded` asserts the decoding rather than `loom:non-utf8-source`; the rule above is therefore met for the first tagged release only once those four have a test, and 14.6 names the tests that were planned and not written.
