...
- `tests/quilts/showcase/`: the quilt to open when you want to see what loom does, copied to `demos/showcase/` by the workspace's `demos/build.py`. Where the synthetic quilt exercises the source contract, this one exercises the records: an invented paper about balanced flows on finite quivers, two invented cited works whose PDFs are compiled by `sources/showcase-works/build.sh` and committed with the quilt (DR-194), a digest extracted from a source and a second built from proposals the author verified, annotations of every kind, severity and state with replies and `cited:` locators into both works, four sessions, one of them discarded, a landmark history with a stamp, a fork and a revert, and the three faults a quilt can be left in. `test_the_showcase_carries_the_faults_it_is_meant_to_show` pins those. It is regenerated after a feature lands, which is what keeps it a picture of loom today.
...
**[decided]** Five workflows in the workspace's `.github/workflows/`, each on push and pull request to the paths it names. `loom-unit.yml` (on `loom/**`) has a `unit` job over a Python matrix (`uv sync`, `ruff check`, `ruff format --check`, `mypy`, `pytest -m "not tex and not paper and not network"`) and an `install-from-clone` job that runs `pipx install .` on a bare runner, then `loom --version`, and checks that `loom doctor` exits 2 there because no TeX is installed. `loom-tex.yml` (on `loom/**`) runs the TeX tier in the `texlive/texlive:latest` container with poppler installed (`uv run pytest -m tex`). `arras-ci.yml` (on `arras/**`): Node 22, `npm ci`, `svelte-check`, vitest, `npm run build`, whose `build/` is set aside and uploaded as the `bundle` artefact before Playwright rebuilds it with a fixture for the e2e and minimal suites. `clients.yml` (on the three clients only) runs the loom-lsp, loom-nvim and loom-vscode suites of 14.6; the clients are deprecated and may break with any loom change (WQ-49), so a loom change does not run them. `mirror-nvim.yml` pushes `loom-nvim/` to its mirror repository only after calling `clients.yml` for the nvim suite. Not built: an Overleaf proxy job (compiling the demo and synthetic masters from the root with an empty environment), a publish job on tags, a prerender job, and a workflow running the dialect validator; the validator runs inside `refresh-fixture.sh` and in loom's `test_fragment_kinds_and_dialect_validity` instead.
...
- `loom-lsp` (`uv run pytest`): twenty-six tests. The server is driven in process against a copy of the synthetic quilt — the published diagnostics match `loom lint --json`, a dangling reference is an error at the command rather than at the line, an unsaved buffer produces diagnostics the disk does not justify and closing it takes them away again, definitions resolve for a reference and an inclusion, references find every site, hover carries the state, the symbol tree nests proofs under their statements, completion offers ids, directive keys, taxa and citekeys in the right places, a code action arrives as an edit for the missing `\uses` and as a confirmed command for the rest, Open in arras names the statement even from its proof, atomize and the id action are offered only where they apply and plan from the unsaved buffer, and workspace symbols, the call hierarchy and inlay hints answer. Four tests cover the position encoding: an astral character, a negotiated code-point encoding, a CRLF file, and a position past the end of a line. One drives the real binary over stdio, and one asserts that the transport flag clients append is accepted.
- `loom-nvim` (headless Neovim under plenary's busted): forty-eight tests over root detection, the key under the cursor, every command's argument vector, the client configuration, the language server's commands, the reshaping commands, the servers a session owns, and TeX's search path following the current buffer. The browser opener is injected and a `python3 -m http.server` stands in for `loom serve`; two tests start loom's own processes when they are on the path, one opening a node on a real `loom serve` over a copy of the demo quilt and one atomizing through the real `loom-lsp` over a copy of the synthetic quilt. Two scripts under `scripts/` drive the real server against a real quilt by hand and assert nothing.
- `loom-vscode` (`npm test`, `@vscode/test-cli`): forty-seven tests in two passes in a real VS Code, forty-three inside a fresh copy of the synthetic quilt, where the client must reach *running* and navigation and atomize answer from the real server, and four in a plain LaTeX folder, where no client may start. The harness makes both folders in a new temp dir on each run. The seven tests that need `loom-lsp` or `loom` skip when `LOOM_LSP` or `LOOM_BIN` is unset.
