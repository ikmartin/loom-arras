...
The **quilt repository** holds the complete project and Loom records and uses ordinary Git remotes, commits, pulls and pushes. Loom manages only the **document workspace**, a source-only Git repository such as Overleaf (DR-300-luisa). `loom sync init --remote overleaf --branch master` pairs it with the quilt; fetch that Git remote before initializing. The configured main document is always selected; `loom sync documents add drafting/toy.tex` persistently selects another live drafting document, `remove` withdraws a secondary selection, and the command with no action lists the selection. Selection only writes `.loom/source-sync.json`: it neither stages, commits nor publishes. Commit source edits normally, then run `loom sync publish` to build the union of committed required inputs and compile every selected document. It refuses uncommitted selected inputs and remote-path collisions, creates a source-only commit at `refs/loom/publication`, and records the producing quilt commit. It never pushes, suggests a push command, or changes the quilt's index or HEAD. `--publish-main main.tex` maps only the primary master; secondary documents retain their quilt-relative paths. Push the prepared ref using ordinary Git when ready. A later `loom sync fetch` recognizes the exact prepared revision as published without Incoming work; other revisions are pinned for Incoming review in Arras. Incorporating a reviewed pull commits the reconciled source and its private sync record separately, without pushing. Mathematical acceptance remains a separate act.
