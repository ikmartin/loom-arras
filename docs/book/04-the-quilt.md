...
For an Overleaf Git project, `loom sync` offers a source-only projection instead (DR-217, DR-227-VelascoL). Work on a private local Git branch containing the full quilt, including `.loom/`. The configured main document is always selected; `loom sync documents add drafting/toy.tex` persistently selects another live drafting document, `remove` withdraws a secondary selection, and the command with no action lists the projection. Selection only writes `.loom/source-sync.json`: it neither commits nor publishes. `loom sync publish --push` sends a separate commit containing the union of the committed inputs reachable from the selected documents, after compiling every selected document from that exact tree. `--publish-main main.tex` maps only the primary master to an existing Overleaf main file; secondary documents retain their quilt-relative paths. A successful push commits the updated private sync record but never stages or commits author source. This does not change the quilt's drafting files. `loom sync fetch` pins the remote commit and rebuilds an Incoming review in Arras without merging it. After reviewing the whole pull, the author incorporates it from Arras, which commits the reconciled source and its private sync record separately. A mathematical acceptance remains a separate act.
