...
**[decided]** `/library`: one row per cited work (DR-206), with what is known of it — what cites it, what has been read of it, what waits on the author — filtered in the URL (`?show=proposed`, `digested`, `held`, `unread`); the list of proposals is this view filtered, not another page. **[decided]** `/library/<citekey>`: the page for a cited work, digested or not, and — with `?page=N` — **the document itself in the viewer** (plan 0.13 item 6), opened split with the discussion beside it, the content pane carrying tabs **Paper** and **Digest**. The URL names the place with the same keys a `cited:` link carries: `page`, `span=A-B` (offsets into the page's committed text), `box=x0,y0,x1,y1` (points, origin top left), `annot=<id>`; a `span`, `box` or `quote` is **lit while the URL carries it** and never recorded, and `annot` focuses that note's mark (DR-210).
...
**[decided]** **On a page of a cited work** the same marks stand (DR-209): a note's rectangles, coloured by its kind, drawn on the drawn pages only; two notes on one place are **one mark carrying the count**, and the box that opens holds both; a result's anchor keeps the neutral tint. Selecting on the page, or drawing a box, opens **the composer at the place**, showing the words loom found before anything is typed, and *note it* records the annotation against the work in the active session. **The margin tick** of item 8 stands beside every annotated line, in a fragment and beside a page alike, on the discussion side and following the swap, carrying the count where several share the line; click selects, double-click travels.
...
**[decided]** A link `cited:<scheme>:<value>?page=N&span=A-B` in a comment, a thread message or a report names a place in a cited work by its global identifier (specs/dialect.md §2.13), in the keys the app's own URLs use — one locator syntax with two prefixes (DR-210; the earlier `#page=`/`#quote=` fragment form is still read). Selecting it **lands in the Library View at the page with the place lit** when the artifact it names is filed here; the viewer of 15.3.7 opens only for a copy that is another version of the work, or none. The fetched PDF is opened only when it is filed under the identifier the link names; when the copy on file is another artifact of the same work, the viewer says the pages may not match and opens it only on request, and when nothing is fetched it says so and links to the identifier's own service, arXiv's PDF at the page where it can (DR-123).
