...
3. Beside `manifest.json` and `fragments/`, a publisher may write `source/<key>.tex`: one file per key holding its own text before macro expansion, fetched lazily for the verbatim view. It is beside the manifest rather than inside it because the manifest is loaded whole on every poll and already runs to hundreds of kilobytes, while source is wanted one key at a time and only when a reader asks (DR-158). A viewer that finds nothing there shows the rendered form and no toggle.
4. Arras is distributed from one build. Loom vendors `build/` under `src/loom/assets/arras/` through `scripts/vendor_arras.py`, which stamps `VERSION` with the arras commit and the interface version, so loom users never install Node; `npm run build:pip` copies the same build into the optional `arras` pip wrapper under `python/`, whose `arras.bundle_path()` loom also honours; the npm package is private. Publishing to PyPI and npm is the user's step (M7).
5. Arras has no runtime dependency on the publisher for **reading**: it polls the manifest, and a corpus on a static host needs nothing else. Writing is the one exception and it is detected rather than assumed — `GET /_api` answers with the capabilities the publisher serves, `loom serve` serves them, a static host answers 404, and a viewer that gets 404 shows no editing affordance at all (DR-161). The same bundle therefore reads a deployed site and edits a served quilt with no build-time flag deciding which.
6. The manifest is loaded whole; fragments are fetched lazily per page and cached in memory under the manifest's hash, and a new hash empties the cache.
7. The corpus is named by `corpus.name`, which the publisher takes from the project's own configuration and falls back to the directory's name. It is what the shell and the browser tab show; `corpus.root_label` remains the default document's title, which is a different thing and is shown where a document is meant.
...
**[decided]** `/node/<key>` for any node, digest node, or labelled proof (the key travels in a rest segment, so `sy-0003/proof` and ids with dots are routes). It is the address of a node item (15.2.4); what the list below calls the context — from the breadcrumb to the diagnostics, with the local graph — is a second item, `/context/<key>`, opened beside the node rather than drawn in a rail (DR-236-ikmartin). Together they show:

- The node fragment: statement, then each proof as a collapsible block, and nothing else in its pane — the tab names the node, and Show ids puts its id and state in the left gutter (DR-242-ikmartin).
- Marks and comment boxes (10.4); nothing below the node lists its annotations again.
- In the context, `from`: the work a digest result was read off and where, the landmark each key's text was recorded at, and tags as links.
...
- The statement's closure: a "read first" count under depends on, and **what this rests on**, the stacked closure, opened in place (DR-160).
- The citations suggested for it, to accept or reject where a publisher is serving (10.4), and a quiet `N discarded — show` when anything on it was discarded.
...
- The local graph, heading the context: the node's neighbourhood one or two dependency steps out, live, each node a link (15.5.1, DR-116).
...
**[decided]** `/library` is the **ledger** (DR-206, DR-239-ikmartin): one row per cited work, one question per column — whether a copy is filed, how many results were read off it, how many of those the corpus leans on, how many statements nobody has vouched for, how many notes and findings on it still await an answer — filtered in the URL to `needs-work` or `proposed`; the list of proposals is this view filtered, not another page. It is a working table and not a second list of works: the list is the side panel's. **[decided]** `/library/<citekey>` addresses a work item (15.2.4): the paper first, then its Digest and its Info as the rail's views (`?view=digest`, `?view=info`). The URL names the place with the same keys a `cited:` link carries: `page`, `span=A-B` (offsets into the page's committed text), `box=x0,y0,x1,y1` (points, origin top left), `annot=<id>`; a `span`, `box` or `quote` is **lit while the URL carries it** and never recorded, and `annot` focuses that note's mark (DR-210).

### 10.2.4 Review panel

**[decided]** `/review` has one view per working document, followed by **Needs review** and **Incoming**. The configured default document opens first; each document table keeps state, basis, cause, review facts, gaps and downstream blocks in rows selected by the owner node's reachability, and computes its counts from those rows. One shared node may therefore appear in two document views without acquiring two review states. Needs review is an ordered quilt-wide block queue with separate pending OK and Requires attention lists; it highlights the cited changed block inside a dependent's current fragment and positions that citation in the visible pane when the block opens. A current pending OK hides dependents whose only causes are indirect through it, without recording acceptance. Incoming displays the pinned collaborator revision and every changed file beside the local incorporation action at the top, with block comparisons and source diffs below. Sessions and runs remain on their existing surfaces, and undigested works remain in Library rather than being repeated under Review.
...
**[decided]** `/blockers` redirects to Review's default working document. Every incomplete key carries its gap and an expandable list of what it blocks in each document table that reaches it (DR-220, DR-225).
...
**[decided]** Marks in fragments render as underlined spans coloured by the kind of their leading comment; a block-level annotation is underlined line by line, and a display ruled beneath (DR-244-ikmartin). A mark shared by several annotations **carries the count**, because two on one sentence is ordinary and stacked translucent highlights muddy immediately. Where the box appears is a display preference (15.3.1, DR-114, DR-202): **`floating`** over the text at the mark, **`margin`** beside the node, where boxes with close anchors push each other down, or **`inline`** in the flow, which is the Authoring View's alone.

**[decided]** **A click opens; hovering never does.** Many boxes may be open at once; a press outside every open box closes them, as do its own × and Escape on the front-most (DR-225). **Expand all is a state, not an action**, opening every annotation at its own mark, in the flow beneath its block whatever the placement (DR-260-ikmartin), so what materialises later opens too; `e` and `h` are its keys, bound only while the text has focus so they never reach the composer, and the rail's `annotations` control is the same state for a reader who wants no key (15.2.5).
...
**[decided]** **On a page of a cited work** the same marks stand (DR-209): a note's rectangles, coloured by its kind, drawn on the drawn pages only; two notes on one place are **one mark carrying the count**, and the box that opens holds both; a result's anchor keeps the neutral tint. Selecting on the page, or drawing a box, opens **the composer at the place**, the selection or the box kept lit on the page while it is open (DR-262-ikmartin), and *note it* records the annotation against the work in the active session. **The same two tools write on a node and on a document** (DR-243-ikmartin): a selection is noted on the result it is in, quoted as its TeX; a box round a labelled equation notes the equation. There is no tick column beside the page (DR-227): the marks are on the page the reader is looking at, and a column of dashes outside its box marked nothing they could not already see.

A node draws no list of its annotations: its marks open them, and `show all annotations` opens every one (DR-242-ikmartin). Discarded records are hidden, and the node's context carries a quiet `N discarded — show` line, drawn only when there are any, which shows them dimmed there.

### 10.4.1 Links

**[decided]** **Two forms name everything a reader can be sent to** (DR-268-ikmartin, specs/dialect.md §2.13). `quilt:KEY[#PLACE]` names what the corpus owns by its fixed key — a node, a document (at a place in it after `#`), an annotation, a session — and `cited:` names a place in a cited work, below. Either may carry no text: the viewer then names what it points at as a reader does, `Theorem 3.1`, `objection on Theorem 2.1`, `main.tex`, so the name stays right when the document is renumbered. Every link, of either form and of the viewer's own paths, is followed by the one rule of 15.2.4, and previews as 15.3.6 says.


**[decided]** A link `cited:<scheme>:<value>?page=N&span=A-B` in a comment, a chat message or a report names a place in a cited work by its global identifier (specs/dialect.md §2.13), in the keys the app's own URLs use — one locator syntax with two prefixes (DR-210; the earlier `#page=`/`#quote=` fragment form is still read). Selecting it **opens the work at the page with the place lit** when the artifact it names is filed here — in the other pane when the link stands in an item, so the text it was followed from stays, and in the focused pane otherwise (15.2.4, DR-233-ikmartin); the viewer of 15.3.7 opens only for a copy that is another version of the work, or none. The fetched PDF is opened only when it is filed under the identifier the link names; when the copy on file is another artifact of the same work, the viewer says the pages may not match and opens it only on request, and when nothing is fetched it says so and links to the identifier's own service, arXiv's PDF at the page where it can (DR-123).
...
The Review table shows the recorded state badge and, on statement rows, separate `proved` and `settled` badges from `derived`; proof rows show only recorded state. Its counts cover the same document-reached population as the table. A `loom:missing-proof` diagnostic appears on its block with a link to the matching row in a document that reaches it, without changing the key's recorded state. Stale `text edit` and direct dependency labels open the current document and a rendered comparison column; a direct dependency citation is highlighted in the document while its comparison is open. Indirect `A via C` labels do not link to A. The comparison joins the document layout, marks only changed source text or math, and scrolls wide equations within their equation area. Accepted math uses the preamble saved at acceptance and is revealed after typesetting. Arras continues polling the published manifest while `loom serve` runs; publishing is triggered by `loom review`, `loom build`, or live `loom serve`, rather than by a standalone scan.
