...
**[decided]** `/blockers`: every key with `\incomplete`, grouped by master, each with the text and a link; then the results downstream of any incomplete key. This is the "incomplete" backlink page of the author's org practice, made from the manifest.

### 10.2.7 Graph

**[decided]** `/graph`: the dependency graph as a layered drawing (ELK.js), layered by section then by longest path; nodes coloured by state and freshness, shaped by style class; edges by kind (statement solid, proof dashed, prose dotted); inclusion shown as grouping, not as edges. Controls: highlight downstream of a node (the unravel set), highlight closure, filter by master, by taxon, by tag, hide external nodes. Click highlights, double-click opens the node page.
...
**[decided]** `/tags` and `/tag/<tag>`; `/taxa` and `/taxon/<slug>`; `/references` (every citekey with or without a digest); `/loose`. A search palette (ninja-keys) over the manifest's search entries (nodes, masters, and threads), grouped by taxon, reachable by keyboard shortcut (⌘K) from every page.
...
**[decided]** Arras polls `/build/manifest.json` every second with `If-None-Match` (**[decided]** `src/lib/manifest/client.svelte.ts`; `loom serve` answers 304 while the manifest is unchanged, so the poll costs nothing) and re-renders the current page when the manifest's content hash changes (M2). That is its only trigger. It does not know what changed or why; a new hash empties the fragment cache and the page re-fetches what it shows. The interface version is checked on every load; a mismatch shows the problems page with one diagnostic and nothing else.
