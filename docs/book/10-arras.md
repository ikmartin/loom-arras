...
**[decided]** `/review`: the read-only view of the ledger and the review records. A header with counts (accepted, stale, draft, incomplete, proved, settled, and live blocks needing classification). A table of keys with: state badge, basis (including an inline marker for a remark or comment whose argument is in the block) and the reason if unclassified, since when, cause if stale (with a link to the diff under `build/diffs/`), latest review and open annotations by kind, detached count, incomplete text, reached-by. Filters: accepted, stale, draft, incomplete, loose, previous-key matches (retired keys), cited results, needs classification, by master, by author, by tag; they live in the URL (`/review?show=classification&document=…`) and each header count toggles its own. With `show=incomplete` the table says what each gap blocks, expandable to the list (DR-118). Undigested citekeys are listed below the unfiltered table (M3). A list of runs and comment sessions with their discard flag. The classification choice is written as `% !LOOM basis:` in drafting source, not through this read-only panel (DR-198, DR-199).
