...
**[decided]** `/review` has exactly three views: **All**, **Needs review**, and **Incoming**. All is the default complete ledger and keeps state, basis, cause, review facts, gaps and downstream blocks in each row. Needs review is an ordered block queue with separate pending OK and Requires attention lists; it highlights the cited changed block inside a dependent's current fragment and positions that citation in the visible pane when the block opens. A current pending OK hides dependents whose only causes are indirect through it, without recording acceptance. Incoming displays the pinned collaborator revision and every changed file beside the local incorporation action at the top, with block comparisons and source diffs below. Specialist states remain visible in All's rows and counts rather than becoming panel filters. Undigested citekeys and sessions follow the All table.
...
**[decided]** `/blockers` redirects to Review's All view. Every incomplete key carries its gap and an expandable list of what it blocks in that complete table (DR-220).
