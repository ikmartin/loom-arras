...
1. **[decided]** For every theorem-like node in `SRC` with an id, the region consisting of the node's environment, any `% !LOOM` directive lines immediately preceding it (its node-level directives), and the proofs attached to it by adjacency that follow it with nothing but whitespace between, widened to whole lines, is written to `nodes/<id>.tex`, and replaced in `DEST` by `\input{nodes/<id>}` on its own line; the blank lines around the region stay where they were, so that paragraphing is unchanged. A theorem-like environment nested inside another node, and a proof attached by enclosure, travel with the enclosing node (DR-41). If an explanatory remark or comment block lies between a statement and its positional proof, atomize refuses that move until the proof title names its target with `\ref`; otherwise moving the blocks would lose the positional association (DR-213). A `%` line between them separates nothing, for atomize as for the scanner that attached the proof: Manolache's `man-0033` has a commented-out older statement there and moves with its proof.
