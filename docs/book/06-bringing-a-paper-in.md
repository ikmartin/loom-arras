...
2. **[decided]** Write the master, flattened, to `<canon>/<name>.tex`: every `\input`, `\include` and `\nest` of a `.tex` file expanded in place, `\nest`'s level shift applied, comments and directives kept (17.13). An inclusion of anything that is not a `.tex` file — a figure's `.pspdftex`, a system file — stays as written, which is why the rest of the closure is still copied.
