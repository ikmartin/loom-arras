...
When an incoming patch has been prepared, `incoming.prepared` carries its absolute `root` and `patch` paths, `incoming` commit, and affected source `paths`. These are instructions for a local served viewer only; a static viewer has no sync capability. An optional top-level `unresolved` array lists blocks awaiting a mathematical decision after incorporation or an earlier edit. Each row has `key`, `status` (`needs-review`, `ok`, or `requires-attention`), `cause` (`incoming-pull` or `earlier-change`), `pull` commit when applicable, `changed_text`, and `invalidated`. `ok` is pending and does not change the key's recorded state.
