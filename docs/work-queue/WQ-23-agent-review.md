...
- **Notation belongs to agent prose only**: reports, explanations, transcript messages; never `nodes/` or `drafts/`. The agent declares a `[notation]` block once per run; arras marks each declared symbol by TeX token inside the run's math, lists them in an expandable panel on the run pane, and previews the definition on hover. A second meaning for a symbol within one run is flagged.
...
The author asked whether an agent should make its own drafts — a document of chosen nodes with comments and suggested rewrites mixed in — and whether that means a new directory, or `drafts/` becoming ephemeral with another directory for real papers.
...
- **No new directory.** `drafts/` holds the papers and is where a master is recognised; `ai/runs/` is already the ephemeral tier, and "an agent writes only under `ai/runs/`" is enforced by the orientation, by `loom ai init --permissions`, and by `loom ai check`. `loom ai promote` is the gate from one tier to the other.
