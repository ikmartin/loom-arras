...
**Answered by [plan 0.10](../plans/0.10-annotations-and-review.md), built 2026-09-18:** editing an annotation and the severity field (both are record-format changes, and the log made them; DR-147); `referee.md`'s re-check convention, which now reads resolve / edit / discard rather than resolve-or-reply; the candidate reference with no bibliography entry, answered cheaply by reference notes (DR-154); the author's severity mode, which is `ai/modes/review.md` (DR-153); and what quick mode is for, which is the register between a clarification that vanishes and a document (DR-153). Nothing on this list is still open; what remains is the three conversations below.

- **Recording what a run was asked** was resolved the other way: `run.toml` records only what nothing can derive, and both the target and the pipeline are derivable — the target from the annotations a run made, the pipeline from the `<mode>-<target>.notes.md` files it wrote (DR-150).
...
- **Most of it exists.** `loom source KEY --closure` prints a key's closure; `loom comment TARGET --run DIR` anchors a comment to a node, so it follows that node into every document and goes stale with it; a `proposal-KEY.diff` in the run holds a rewrite, and `loom compile KEY --with FILE` compiles it as if it were accepted.
...
- **A proof edit:** the agent leaves `proposal-KEY.diff`; `loom compile KEY --with DIFF` previews it while the node is untouched; `git apply` applies it; the key goes `accepted, stale` on its own; `loom accept KEY`. The referee and simplify modes already require the agent to record the `compile --with` result.
