...
**Still open, and the reason this is an item rather than a plan:** the file's form (a `.tex` spine, or a list of keys with prose that loom renders); how its node rewrites and new nodes are carried (the existing proposal diffs and drafts, or inside it); how it is compared against the section it would replace; how it is accepted, whether as one editor edit or piece by piece; and whether a comment on it uses the same master-path target.

**All five were put to the author on 2026-09-18 and all five were deferred, with a reframing:** *this is a special case of giving the agent its own parallel quilt.* The five questions are therefore not waiting on answers but on that design, and each of them changes shape under it rather than being settled by it — a proposal would be an ordinary master in the agent's own quilt, a rewrite an ordinary node edit, the comparison a matter of two scan roots rather than a form loom must learn to flatten, and the comment target an ordinary master path. Do not answer them piecemeal before the parallel quilt is designed; that is what would produce a format loom has to carry forever.

One fact from that session is recorded here because it cost something to establish: the item's claim that *"`loom linearize` refuses a file that is not a master"* was wrong twice. It refuses a file that is not **scanned** (`cli/history_cmds.py:500`), which is a different and larger refusal, since the scanner skips `ai/` entirely; and it is the wrong command regardless, because it writes into the quilt, supersedes the spine and every file it inlined, and is denied to the agent. The engine beneath it, `flatten(root, rel)`, is pure and needs nothing scanned. Reading a whole document is now `loom source PATH` (DR-155, closed as WQ-35), which is the read-only half and is available to whatever this item becomes.
