...
test.describe('links into cited works', () => {
	test('a link in a comment opens the work beside at its page, and the node stays', async ({ page }) => {
		// A copy on this machine opens as the work itself, not in a modal (plan 0.13 item 6); and a link is a connection
		// between two texts, so it opens in the other pane and the node it was followed from is still there (plan 0.13.3 W7).
		await serve(page, (m) => {
...
test.describe('the digest view', () => {
	...
	test('merges a proposal into the page it is about, with both texts beside each other', async ({ page }) => {
		...
		// the heading and its count, and no paragraph explaining what a proposal is: that is the book's (G14)
