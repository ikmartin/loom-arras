...
# `annotations/` holds the log every `loom comment` appends to, which is the agent's own sanctioned write and not a write outside its session. `.loom/` is skipped wholesale, which is where the session's own directory lives.
