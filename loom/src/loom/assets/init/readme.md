...
- **Loom never edits a file you wrote.** Its own data lives in `.loom/` (the acceptance ledger and the history of every key), `annotations/log.jsonl` (every comment, reply and finding, appended and never rewritten), `ai/runs/` (one directory per agent session), and `build/` (everything derived; delete it whenever you like).
