...
- **Loom edits a file you wrote only when you explicitly click Incorporate pull in a locally served Incoming review.** That action applies the exact displayed collaborator patch and creates local commits; it neither pushes nor accepts mathematics. Its own data lives in `.loom/` (the acceptance ledger and the history of every key), `annotations/log.jsonl` (every comment, reply and finding, appended and never rewritten), `ai/runs/` (one directory per agent session), and `build/` (everything derived; delete it whenever you like).
