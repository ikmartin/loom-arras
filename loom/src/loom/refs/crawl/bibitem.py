"""Formatted bibliographies: `\\bibitem`s wherever they sit in a paper's source (book 8.13.3).
...
from loom.refs.resolve import Query
...
def query(item: BibItem) -> Query:
    ...
    Without a marked title the whole text stands for the title, which a bibliographic search tolerates and loom's scoring then judges.
    """
    title = ""
    head = item.text
