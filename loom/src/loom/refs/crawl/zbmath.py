"""zbMATH Open: MSC codes and resolved reference lists (book 8.13.2).

Looked up by DOI, arXiv number or Zbl number through `_search`, by title and first author for a work it does not know by identifier (a book cited by one edition's or one chapter's DOI, typically), and by zbMATH's own document number for a reference zbMATH matched. A record's references carry the DOI and the document number of the works zbMATH matched them to, with their MSC codes; old papers often have none (Graber–Pandharipande 1999), recent published ones many (Aranha et al. 2025: 46, 30 matched). Data CC-BY-SA 4.0; one request a second.
"""
...
from loom.refs.crawl.net import NotFound, Service
from loom.refs.crawl.work import Reference, norm
from loom.refs.resolve import STRONG, Query, _plain, _surname, score
    ...
    for ref in r.get("references") or []:
        ...
        refs.append(
            Reference(
                ...
                text=(ref.get("text") or "").strip() or None,
...
class Zbmath:
    ...
    def by_document(self, document: int) -> ZbRecord | None:
        """The record for zbMATH's own document number."""
        try:
    ...
    def by_title(self, title: str, authors: list[str], year: int | None) -> ZbRecord | None:
        """The record matching a title, first author and year strongly, as `loom refs resolve` scores a match; None without an author, since a title alone is never strong."""
