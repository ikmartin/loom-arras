"""Subjects and categories: which works a crawl keeps (book 8.13.1).

`[crawl] subjects` lists MSC families, the first three characters of a code (`14N` for `14N35`); a work zbMATH has classified is kept when any of its codes is in one. `[crawl] categories` lists arXiv categories; a work with no MSC code, typically a recent preprint, is kept when its primary category is one of them. Both are the author's: loom holds no correspondence between the two schemes, since any it held would suit some fields and silently lose papers in others.
"""
