"""OpenAlex: reference lists zbMATH lacks, open copies of published papers, and DOIs (book 8.13.2).

One work by DOI is free; a batch of up to 50 by OpenAlex id is one list call ($0.0001): Graber–Pandharipande's 19 references came back as 18 works, 16 with DOIs and 5 with an open PDF. An arXiv preprint's record lists no references. A key is sent as a bearer header from LOOM_OPENALEX_KEY, so it never appears in a URL or names a cache file; without one, requests draw on the small anonymous allowance. Data CC0.
"""
...
from loom.refs.crawl.work import norm
...
class OpenAlex:
    ...
    def by_id(self, ident: str) -> OaRecord | None:
        """The record for a DOI or an arXiv number (through arXiv's DOI)."""
