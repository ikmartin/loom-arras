...
def _import(base: Path, results: str = RESULTS) -> None:
    p = paper_dir(base, results)
    ok("init", str(base / "q"), "--from", str(p / "main.tex"), "--prefix", "pp", "--yes", cwd=base)


def imported(tmp_path: Path, results: str = RESULTS) -> Path:
    """The paper beside a quilt that imported it; the default paper from a per-worker template."""
    if results == RESULTS:
        templated("paper-imported", tmp_path, _import)
    else:
        _import(tmp_path, results)
    return tmp_path / "q"
...
def test_import_leaves_the_paper_directory_and_no_scratch_behind(tmp_path: Path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """The identity test compiles a copy, never the author's directory, and removes its scratch directory from $TMPDIR."""
    import tempfile
...
def test_id_prints_patch_and_to_writes_copy(tmp_path: Path) -> None:
    ...
    ok("id", "nodes/extra.tex", "--to", str(tmp_path / "extra-labelled.tex"), cwd=q)
    assert "\\label{pp-" in (tmp_path / "extra-labelled.tex").read_text()
...
def test_live_makes_a_superseded_document_define_again(tmp_path: Path) -> None:
    ...
    refused("live", "drafting/main.tex", cwd=q, code=2, match="is not superseded")  # not superseded any more
...
def test_linearize_refuses_shared_nodes_keeps_or_forks_them(tmp_path: Path) -> None:
    ...
    refused(
        ...
        "drafting/talk.tex",
        "--to",
        "drafting/talk-flat.tex",
        "--no-check",
        cwd=q,
        code=1,
        match="also included by drafting/spine.tex",
    )
    assert not (q / "drafting" / "talk-flat.tex").exists()

    ok("linearize", "drafting/talk.tex", "--to", "drafting/talk-kept.tex", "--keep-shared", "--no-check", cwd=q)
...
def widget(q: Path) -> str:
    """The key of the Widget definition in drafting/main.tex, the environment the one-key atomize tests move."""
    nodes = scan(load_quilt(q)).assembly.nodes
    return the(
        nodes, lambda k: nodes[k].env == "definition" and nodes[k].file == "drafting/main.tex", "Widget definition"
    )
...
def test_atomize_key_refuses_what_it_cannot_move(tmp_path: Path) -> None:
    ...
    r = refused("atomize", "--key", unlabelled, cwd=q, code=1, match="loom:atomize-unlabelled")
    assert "loom id --next" in r.output
...
def test_atomize_key_moves_an_attached_proof_with_its_statement(tmp_path: Path) -> None:
    ...
    ok("atomize", "--key", proof_key, cwd=q)
...
def test_an_indented_heading_ends_the_section_before_it() -> None:
    """A section's span ends where the next heading begins; one space before `\\section` must not pull that heading into the previous section, or the two moves overlap and the rest of the paper drops out of its spine."""
