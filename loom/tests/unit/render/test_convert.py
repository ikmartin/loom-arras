...
def test_convert_refs_cites_footnote_url() -> None:
    ...
    assert re.search(
        r'<a class="ref ref-dangling" data-target="nope" data-tex="\\ref\{nope\}" data-at="\d+">\?\?</a>', out
    )
    assert 'data-citekey="Man12" data-tex="\\cite[Theorem 4.1]{Man12}"' in out
    assert 'data-postnote="Theorem 4.1" data-target="Man12-thm-4.1"' in out
