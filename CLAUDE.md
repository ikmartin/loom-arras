...
`loom-arras/` is one repository holding `loom` (Python CLI, in `loom/`), `arras` (Svelte viewer, in `arras/`), the three editor clients `loom-lsp/` (a language server), `loom-nvim/` (a Neovim plugin) and `loom-vscode/` (a VS Code extension), and the documents that govern them all: the design book, the interface specification, the plans, the work queue, the deviations log, and the committed example quilts under `demos/`. Each tool keeps its own build, tests and release; `loom-nvim/` is mirrored to `ikmartin/loom-nvim` by a workflow, because a plugin manager installs from a repository whose root is the plugin.
