...
Nothing here is a supported input. The point is what the tools do when the input is not supported: **refuse clearly, degrade visibly, or fail in a way nobody can debug.** Only the third is a bug, and the report in `records/hostile-demo.md` says which is which.
