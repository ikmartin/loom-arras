...
\section{The fixed stack and its deformation theory}
...
\begin{definition}\cite[Definition 2.1]{romagny_GroupActionsStacks2005}
...
\end{definition}
\begin{remark}\label{rmk:description-of-homotopy-fixed-stack}
...
\begin{example}\label{example:BGm-and-its-fixed-stack}
...
\begin{lemma}\cite[Lemma 4.1.2]{romagny_AlgebraicitySmoothnessFixed2022}
...
\subsection{Fixed stacks for torus actions}
\begin{lemma}
...
\begin{proof}$ $
  ...
  \end{enumerate}
\end{proof}
\begin{remark}\label{rmk:algebraicity-is-representability}
...
\begin{lemma}\label{lem:two-term-truncation-of-fixed-locus-cotangent-complex-no-fixed-part}
...
\begin{lemma}\label{lem:obstruction-theory-for-the-fixed-locus}
...
\subsection{Non-algebraicity of the fixed stack}\label{sec:non-algebraicity-of-the-fixed-stack}

\begin{example}\label{example:affine-diagonal-is-necessary}
...
\begin{remark}\label{rmk:effectivity-for-algebraic-spaces}
...
\subsection{Remaining facts from Tom's note that I haven't fit in anywhere else yet so they get their own subsection}


\begin{proposition}\label{prop:condition-for-monomorphism}
...
\section{The residue map for Artin stacks}
...
\begin{prop}[\cite{edidin-graham_LocalizationEquivariantIntersection1998}]\label{prop:edidin-graham-residue-identity}
...
\subsection{Euler classes of vector bundle stacks}\label{sec:euler-classes-of-vector-bundle-stacks}
...
\begin{definition}\label{defn:euler-class-of-vector-bundle-stack}
...
\begin{lemma}\label{lem:euler-class-well-defined}
...
\subsection{The residue map}\label{sec:the-residue-map}
...
\begin{remark}\label{rmk:embedding-data-is-POT}
...
\begin{lemma}[(Existence of embedding data.)]\label{lem:existence-of-pot-for-fixed-locus-map}
...
\begin{lemma}[Independence of the residue map]\label{lem:independence-of-Res-map-from-choice}
...
\begin{remark}\label{rmk:two-modifications-after-the-residue-independence-lemma-statement}
...
\begin{lemma}[Residue independence for honest vector bundles]\label{lem:independence-for-honest-vector-bundles}
...
\begin{lemma}\label{lem:torsor-lemma}
  Suppose
  \begin{align*}
...
\begin{proof}[Proof of Lemma~\ref{lem:independence-of-Res-map-from-choice}]
  ...
  and we're done.
\end{proof}
\begin{definition}\label{defn:residue-map}
...
\begin{proposition}\label{prop:virtual-pullback-commutes-with-residue}
...
\begin{lem}[Obstruction theory for $\widetilde \pi:\widetilde M\to \mathcal Z)$]\label{lem:obstruction-theory-for-tilde-M}
...
\begin{proposition}\label{prop:compatible-triples-for-main-proof}
...
\begin{theorem}\label{thm:main-localization-formula}
...
\section{Appendix: Three fixed stacks, a disambiguation}
...
\begin{definition}\cite{aranha-khan-latyntsev-etal_VirtualLocalizationRevisited2025}
...
\begin{definition}\label{def:stabilizer-fixed-stack}
...
\end{definition}
\begin{remark}(\cite[Remark A.44]{aranha-khan-latyntsev-etal_VirtualLocalizationRevisited2025})
...
\begin{theorem}\label{thm:reconciliation-of-fixed-stacks-for-tori}
